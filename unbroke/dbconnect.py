...
def getbudget(user, year, month):
    ...
    query = c.execute('Select sum(Amount) from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Group By Deposits.UserID;', (user, year, month))
    for row in c:
        budget.append(row)
    while len(budget) < 2:
        budget.append('0')
    ...
    query = c.execute('Select sum(Amount) from Expenses Inner Join Users ON Expenses.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Group By Expenses.UserID;', (user, year, month))
    for row in c:
        budget.append(row)
    while len(budget) < 3:
        budget.append('0')
...
def getdeposits(user, year, month):
    ...
    query = c.execute('Select * from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Order by Date;',
    (user, year, month))
    for row in c:
        d.append(row)
    conn.close()
...
def getdeposits2(user, year, month):
    ...
    c = conn.cursor()
    query = c.execute('Select * from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Order by Date Desc;',
    (user, year, month))
    for row in c:
        d.append(row)
    conn.close()
...
def getdeposits3(user, year, month, desc):
    ...
    c = conn.cursor()
    querystring = 'Select * from Deposits Inner Join Users ON Deposits.UserID= Users.UserID where Username ="'+user+'" and substring(Date from 1 for 4) ="'+year+'" and substring(Date from 6 for 2) ="'+month+'"'
    for descrip in desc:
...
def getexpenses3(user, year, month, desc):
    ...
    c = conn.cursor()
    querystring = 'Select * from Expenses Inner Join Users ON Expenses.UserID= Users.UserID where Username ="'+user+'" and substring(Date from 1 for 4) ="'+year+'" and substring(Date from 6 for 2) ="'+month+'"'
    for descrip in desc:
        querystring += ' and Description ="'
        querystring += descrip + '"'   
    querystring += ' Order by Date Desc;'
...
def getdeposits4(user, year, month):
    ...
    c = conn.cursor()
    query = c.execute('Select * from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Order by Amount;',
    (user, year, month))
    for row in c:
        d.append(row)
    conn.close()
...
def getdeposits5(user, year, month):
    ...
    c = conn.cursor()
    query = c.execute('Select * from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s Order by Amount Desc;',
    (user, year, month))
    for row in c:
        d.append(row)
    conn.close()
...
def getddescs(user, year, month):
    ...
    query = c.execute('Select distinct Description from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Username = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s ;',
    (user, year, month))
    for row in c:
        d.append(row)
    conn.close()
...
def getdtotal2(userID, year, month):
    ...
    c = conn.cursor()
    query = c.execute('Select sum(Amount) from Deposits Inner Join Users ON Deposits.UserID= Users.UserID\
     where Deposits.UserID = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s ;',
    (userID, year, month))
    for row in c:
        total.append(row)
    conn.close()
...
def getetotal2(userID, year, month):
    ...
    c = conn.cursor()
    query = c.execute('Select sum(Amount) from Expenses Inner Join Users ON Expenses.UserID= Users.UserID\
     where Expenses.UserID = %s and substring(Date from 1 for 4) = %s and substring(Date from 6 for 2) = %s ;',
    (userID, year, month))
    for row in c:
        total.append(row)
    conn.close()
